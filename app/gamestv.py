...
def editComment(comment, matchId, comm_id):
    ...
    opener.addheaders = [('Cookie', config.gtvcookie)]
    params = urllib.parse.urlencode(
        {'comm_text': comment, 'comm_replyto': 0, 'comm_id': comm_id, 'comm_add': 'Edit'}).encode('UTF-8')
    url = urllib.request.Request("https://www.gamestv.org/event/" + str(matchId) + '/?id=' + str(matchId), params)
...
def getMatchDemosId(match_id):
    # return 37408
    opener = urllib.request.build_opener()
    opener.addheaders = [('Cookie', config.gtvcookie)]
    html = opener.open("https://www.gamestv.org/match/replay/" + str(match_id)).read().decode('iso-8859-1')
    demosId = re.search(r'<input type="radio" name="demoid" value="(\d+)', html)
    if demosId is not None:
...
def getDemosDownloadLinks(matchId):
    ...
    link = list(filter(lambda x: re.match('\/files\/\d+\-tv\-demo', x),
                       root.xpath('//table[@class="matchDetails"]/tr/td/a/@href')))[0]
