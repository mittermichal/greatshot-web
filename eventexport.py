...
class EventExport:
  ...
  def export(self):
    ...
    open('events/init.cfg','w').write('exec_at_time '+str(self.events[0]['time'])+' events/'+str(self.events[0]['time'])+'\n')
    ...
    for event in self.events:
      open('events/'+str(prev_event['time'])+'.cfg','w').write('echo "'+ prev_event['msg'] +'"\nexec_at_time '+str(event['time'])+' events/'+str(event['time'])+'\n')
      prev_event=event
